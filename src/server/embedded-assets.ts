...
`,
  ...
  'dashboard.js': {
    content: `import{d as ye,r as g,o as be,a as _e,n as E,_ as ke,c as s,b as t,t as u,e as m,f as we,g as z,F as _,h as N,w as O,i as X,v as Y,j as $,k as l,l as M,m as Ce}from"./chunks/_plugin-vue_export-helper-Cvoq67hi.js";const Se=ye({__name:"Dashboard",setup(ae,{expose:n}){n();const q=g("Loading..."),e=g(""),k=g(!1),j=g([]),o=g(null),p=g("Never"),A=g({}),y=g(new Set),U=g({}),D=g(localStorage.getItem("dmux-theme")||"dark"),P=g(new Set),b=g(new Set),B=g(!1),T=g(""),w=g(null),J=g(!1),F=g([]),I=g(!1),x=g("prompt"),ne=g([]),H=g({}),C=g(null),h=g(null),K=g(!1),S=g(!1);let v=null;const ie=()=>{D.value=D.value==="dark"?"light":"dark",localStorage.setItem("dmux-theme",D.value),document.documentElement.setAttribute("data-theme",D.value)},se=a=>{P.value.has(a)?P.value.delete(a):P.value.add(a),P.value=new Set(P.value)},le=()=>{B.value=!0,T.value="",w.value=null,F.value=[],I.value=!1,x.value="prompt",E(()=>{const i=document.getElementById("pane-prompt");i&&i.focus()});const a=i=>{i.key==="Escape"&&(Q(),document.removeEventListener("keydown",a))};document.addEventListener("keydown",a)},Q=()=>{B.value=!1,T.value="",w.value=null,F.value=[],I.value=!1,x.value="prompt"},ee=async()=>{if(!(x.value==="prompt"&&!T.value.trim())&&!(x.value==="agent"&&!w.value))try{J.value=!0;const a={prompt:T.value.trim()};w.value&&(a.agent=w.value);const r=await(await fetch("/api/panes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})).json();r.needsAgentChoice?(I.value=!0,F.value=r.availableAgents,x.value="agent"):Q()}catch(a){console.error("Failed to create pane:",a),alert("Failed to create pane")}finally{J.value=!1}},ce=a=>{w.value=a,ee()},R=a=>{q.value=a.projectName||"dmux",e.value=a.sessionName||"",k.value=!0,j.value=a.panes||[],o.value=new Date,p.value="Just now";for(const i of j.value)H.value[i.id]||te(i.id)},V=()=>{v&&v.close();try{v=new EventSource("/api/panes-stream"),v.onmessage=a=>{try{const i=JSON.parse(a.data);i.type==="init"||i.type==="update"?R(i.data):i.type==="heartbeat"&&console.debug("SSE heartbeat received")}catch(i){console.error("Failed to parse SSE message:",i)}},v.onerror=a=>{console.error("SSE connection error:",a),k.value=!1,setTimeout(()=>{(v==null?void 0:v.readyState)===EventSource.CLOSED&&(console.log("Reconnecting to SSE stream..."),V())},5e3)},v.onopen=()=>{console.log("SSE connection established"),k.value=!0}}catch(a){console.error("Failed to connect to SSE stream:",a),k.value=!1}},re=async()=>{try{const i=await(await fetch("/api/panes")).json();R(i)}catch(a){console.error("Failed to fetch panes:",a),k.value=!1}},te=async a=>{try{const r=await(await fetch(\`/api/panes/\${a}/actions\`)).json();console.log(\`Fetched actions for pane \${a}:\`,r),H.value[a]=r.actions||[]}catch(i){console.error(\`Failed to fetch actions for pane \${a}:\`,i)}},de=a=>{C.value===a?C.value=null:C.value=a},ue=async(a,i)=>{var r;try{K.value=!0,C.value=null;const d=await(await fetch(\`/api/panes/\${a.id}/actions/\${i.id}\`,{method:"POST"})).json();if(d.requiresInteraction){let f={};d.interactionType==="confirm"?f={type:"confirm",title:d.title||"Confirm",message:d.message,...d.confirmData}:d.interactionType==="choice"?f={type:"choice",title:d.title||"Choose",message:d.message,...d.choiceData}:d.interactionType==="input"&&(f={type:"input",title:d.title||"Input",message:d.message,...d.inputData,inputValue:((r=d.inputData)==null?void 0:r.defaultValue)||""},E(()=>{const G=document.querySelector(".dialog-input");G&&(G.focus(),G.select())})),f.paneId=a.id,h.value=f}}catch(c){console.error("Failed to execute action:",c),alert("Failed to execute action")}finally{K.value=!1}},L=()=>{h.value=null},ge=async a=>{var i;if(h.value)try{S.value=!0;const c=await(await fetch(\`/api/callbacks/confirm/\${h.value.callbackId}\`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({confirmed:a})})).json();if(c.requiresInteraction){let d={};c.interactionType==="confirm"?d={type:"confirm",title:c.title||"Confirm",message:c.message,...c.confirmData}:c.interactionType==="choice"?d={type:"choice",title:c.title||"Choose",message:c.message,...c.choiceData}:c.interactionType==="input"&&(d={type:"input",title:c.title||"Input",message:c.message,...c.inputData,inputValue:((i=c.inputData)==null?void 0:i.defaultValue)||""},E(()=>{const f=document.querySelector(".dialog-input");f&&(f.focus(),f.select())})),h.value=d}else L()}catch(r){console.error("Failed to confirm action:",r),alert("Failed to complete action")}finally{S.value=!1}},me=async a=>{var i;if(h.value)try{S.value=!0;const c=await(await fetch(\`/api/callbacks/choice/\${h.value.callbackId}\`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({optionId:a})})).json();if(c.requiresInteraction){let d={};c.interactionType==="confirm"?d={type:"confirm",title:c.title||"Confirm",message:c.message,...c.confirmData}:c.interactionType==="choice"?d={type:"choice",title:c.title||"Choose",message:c.message,...c.choiceData}:c.interactionType==="input"&&(d={type:"input",title:c.title||"Input",message:c.message,...c.inputData,inputValue:((i=c.inputData)==null?void 0:i.defaultValue)||""},E(()=>{const f=document.querySelector(".dialog-input");f&&(f.focus(),f.select())})),h.value=d}else L()}catch(r){console.error("Failed to select choice:",r),alert("Failed to complete action")}finally{S.value=!1}},pe=async()=>{var a;if(h.value)try{S.value=!0;const r=await(await fetch(\`/api/callbacks/input/\${h.value.callbackId}\`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({value:h.value.inputValue})})).json();if(r.requiresInteraction){let c={};r.interactionType==="confirm"?c={type:"confirm",title:r.title||"Confirm",message:r.message,...r.confirmData}:r.interactionType==="choice"?c={type:"choice",title:r.title||"Choose",message:r.message,...r.choiceData}:r.interactionType==="input"&&(c={type:"input",title:r.title||"Input",message:r.message,...r.inputData,inputValue:((a=r.inputData)==null?void 0:a.defaultValue)||""},E(()=>{const d=document.querySelector(".dialog-input");d&&(d.focus(),d.select())})),h.value=c}else L()}catch(i){console.error("Failed to submit input:",i),alert("Failed to complete action")}finally{S.value=!1}},ve=async(a,i)=>{try{b.value.add(a.id),b.value=new Set(b.value);const r=i.keys||[i.action];for(const c of r)await fetch(\`/api/keys/\${a.id}\`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:c})});setTimeout(()=>{b.value.delete(a.id),b.value=new Set(b.value)},1500)}catch(r){console.error("Failed to select option:",r),b.value.delete(a.id),b.value=new Set(b.value)}},he=async a=>{const i=A.value[a.id];if(!(!i||!i.trim()))try{y.value.add(a.id),y.value=new Set(y.value);for(const r of i)await fetch(\`/api/keys/\${a.id}\`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:r})});await fetch(\`/api/keys/\${a.id}\`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:"Enter"})}),U.value[a.id]=i.substring(0,50)+(i.length>50?"...":""),A.value[a.id]="",setTimeout(()=>{delete U.value[a.id],y.value.delete(a.id),y.value=new Set(y.value)},3e3)}catch(r){console.error("Failed to send prompt:",r),y.value.delete(a.id),y.value=new Set(y.value)}},fe=a=>{const i=a.target;i.style.height="auto",i.style.height=i.scrollHeight+"px"},W=()=>{v&&(v.close(),v=null)},Z=a=>{const i=a.target;C.value&&!i.closest(".action-menu-btn")&&!i.closest(".action-menu-dropdown")&&(C.value=null)};be(()=>{document.documentElement.setAttribute("data-theme",D.value),V(),document.addEventListener("click",Z),document.addEventListener("visibilitychange",()=>{document.hidden?W():V()})}),_e(()=>{W(),document.removeEventListener("click",Z)});const oe={projectName:q,sessionName:e,connected:k,panes:j,lastUpdate:o,timeSinceUpdate:p,promptInputs:A,sendingPrompts:y,queuedMessages:U,theme:D,expandedPrompts:P,loadingOptions:b,showCreateDialog:B,newPanePrompt:T,newPaneAgent:w,creatingPane:J,availableAgents:F,needsAgentChoice:I,createStep:x,actions:ne,paneActions:H,showActionMenu:C,actionDialog:h,executingAction:K,actionDialogLoading:S,get eventSource(){return v},set eventSource(a){v=a},toggleTheme:ie,togglePrompt:se,openCreateDialog:le,closeCreateDialog:Q,createPane:ee,selectAgent:ce,updatePanesFromData:R,connectToStream:V,fetchPanes:re,fetchPaneActions:te,toggleActionMenu:de,executeAction:ue,closeActionDialog:L,confirmAction:ge,selectChoice:me,submitInput:pe,selectOption:ve,sendPrompt:he,autoExpand:fe,disconnectStream:W,handleClickOutside:Z};return Object.defineProperty(oe,"__isScriptSetup",{enumerable:!1,value:!0}),oe}}),De={class:"session-info"},Pe=["title"],xe={key:0,xmlns:"http://www.w3.org/2000/svg",viewBox:"0 0 24 24"},Ae={key:1,xmlns:"http://www.w3.org/2000/svg",viewBox:"0 0 24 24"},Te={key:0},Ee={class:"container"},Ne={class:"actions-bar"},Oe=["disabled"],Me={key:0,class:"no-panes"},je={key:1,class:"panes-grid"},Fe={class:"pane-header"},Ie={class:"pane-header-content"},Ve=["href"],Le={class:"pane-title"},ze={class:"pane-meta"},qe={class:"pane-id"},Ue=["onClick"],Be={key:0,class:"action-menu-dropdown"},Je=["onClick","disabled"],He={class:"action-icon"},Ke={class:"action-label"},Qe={class:"pane-prompt-section"},Re=["onClick"],We={class:"prompt-text"},Ze={class:"expand-icon"},Ge={key:0,class:"pane-prompt-full"},Xe={key:1,class:"agent-summary"},Ye={key:0,class:"options-dialog"},$e={class:"options-question"},et={key:0,class:"options-warning"},tt={key:1,class:"analyzing-state"},ot={key:2,class:"options-buttons"},at=["onClick","disabled"],nt={class:"prompt-input-wrapper"},it=["onUpdate:modelValue","placeholder","disabled"],st=["onClick","disabled","title"],lt={key:0,class:"button-loader"},ct={key:1,xmlns:"http://www.w3.org/2000/svg",viewBox:"0 0 988.44 1200.05"},rt={key:0,class:"queued-message"},dt={key:2,class:"dev-server-status"},ut=["href"],gt={class:"action-dialog"},mt={key:0},pt=["onKeydown"],vt={class:"dialog-buttons"},ht=["disabled"],ft={key:1},yt={class:"agent-choices"},bt=["onClick","disabled"],_t={key:0,class:"action-dialog"},kt={key:0,class:"dialog-loading"},wt={key:1,class:"dialog-buttons"},Ct=["disabled"],St=["disabled"],Dt={class:"action-dialog"},Pt={key:0},xt={key:1,class:"dialog-loading"},At={key:2},Tt={class:"choice-options"},Et=["onClick","disabled"],Nt={class:"choice-label"},Ot={key:0,class:"choice-description"},Mt={class:"dialog-buttons"},jt=["disabled"],Ft={class:"action-dialog"},It={key:0},Vt={key:1,class:"dialog-loading"},Lt={key:2},zt=["placeholder"],qt={class:"dialog-buttons"},Ut=["disabled"],Bt=["disabled"];function Jt(ae,n,q,e,k,j){return l(),s(_,null,[t("header",null,[n[7]||(n[7]=t("img",{src:"https://cdn.formk.it/dmux/dmux.png",alt:"dmux",class:"logo"},null,-1)),t("h1",null,u(e.projectName),1),t("div",De,[t("button",{onClick:e.toggleTheme,class:"theme-toggle",title:e.theme==="dark"?"Switch to light mode":"Switch to dark mode"},[e.theme==="dark"?(l(),s("svg",xe,[...n[5]||(n[5]=[t("path",{d:"M12 2.25a.75.75 0 01.75.75v2.25a.75.75 0 01-1.5 0V3a.75.75 0 01.75-.75zM7.5 12a4.5 4.5 0 119 0 4.5 4.5 0 01-9 0zM18.894 6.166a.75.75 0 00-1.06-1.06l-1.591 1.59a.75.75 0 101.06 1.061l1.591-1.59zM21.75 12a.75.75 0 01-.75.75h-2.25a.75.75 0 010-1.5H21a.75.75 0 01.75.75zM17.834 18.894a.75.75 0 001.06-1.06l-1.59-1.591a.75.75 0 10-1.061 1.06l1.59 1.591zM12 18a.75.75 0 01.75.75V21a.75.75 0 01-1.5 0v-2.25A.75.75 0 0112 18zM7.758 17.303a.75.75 0 00-1.061-1.06l-1.591 1.59a.75.75 0 001.06 1.061l1.591-1.59zM6 12a.75.75 0 01-.75.75H3a.75.75 0 010-1.5h2.25A.75.75 0 016 12zM6.697 7.757a.75.75 0 001.06-1.06l-1.59-1.591a.75.75 0 00-1.061 1.06l1.59 1.591z"},null,-1)])])):(l(),s("svg",Ae,[...n[6]||(n[6]=[t("path",{d:"M9.528 1.718a.75.75 0 01.162.819A8.97 8.97 0 009 6.5a9 9 0 009 9 8.97 8.97 0 003.963-.69.75.75 0 01.981.98 10.503 10.503 0 01-9.694 6.46c-5.799 0-10.5-4.701-10.5-10.5 0-4.368 2.667-8.112 6.46-9.694a.75.75 0 01.818.162z"},null,-1)])]))],8,Pe),e.sessionName?(l(),s("span",Te,u(e.sessionName),1)):m("v-if",!0),t("span",{class:"status-indicator",style:we({color:e.connected?"#4ade80":"#f87171"})},"●",4)])]),t("div",Ee,[t("main",null,[t("div",Ne,[t("button",{onClick:e.openCreateDialog,class:"create-pane-button",disabled:e.creatingPane},[...n[8]||(n[8]=[t("svg",{xmlns:"http://www.w3.org/2000/svg",viewBox:"0 0 24 24",fill:"currentColor"},[t("path",{d:"M12 4.5v15m7.5-7.5h-15",stroke:"currentColor","stroke-width":"2","stroke-linecap":"round","stroke-linejoin":"round"})],-1),z(" Create New Pane ",-1)])],8,Oe)]),e.panes.length===0?(l(),s("div",Me,[...n[9]||(n[9]=[t("p",null,"No dmux panes active",-1),t("p",{class:"hint"},\`Click "Create New Pane" above or press 'n' in dmux\`,-1)])])):(l(),s("div",je,[(l(!0),s(_,null,N(e.panes,o=>(l(),s("div",{key:o.id,class:"pane-card"},[t("div",Fe,[t("div",Ie,[t("a",{href:"/panes/"+o.id,class:"pane-title-link"},[t("span",Le,u(o.slug),1),n[10]||(n[10]=t("span",{class:"pane-arrow"},"→",-1))],8,Ve),t("div",ze,[t("span",{class:M(["pane-agent",o.agent||""])},u(o.agent||"unknown"),3),t("span",qe,u(o.paneId),1)])]),t("button",{onClick:p=>e.toggleActionMenu(o.id),class:"action-menu-btn",title:"Actions"},[...n[11]||(n[11]=[t("span",null,"⋮",-1)])],8,Ue)]),m(" Action Menu Dropdown "),e.showActionMenu===o.id&&e.paneActions[o.id]?(l(),s("div",Be,[(l(!0),s(_,null,N(e.paneActions[o.id],p=>(l(),s("button",{key:p.id,onClick:A=>e.executeAction(o,p),class:"action-menu-item",disabled:e.executingAction},[t("span",He,u(p.icon||"•"),1),t("span",Ke,u(p.label),1)],8,Je))),128))])):m("v-if",!0),t("div",Qe,[n[12]||(n[12]=t("div",{class:"prompt-label"},"Initial Prompt",-1)),t("div",{class:M(["pane-prompt-preview",{expanded:e.expandedPrompts.has(o.id)}]),onClick:p=>e.togglePrompt(o.id)},[t("span",We,u(o.prompt||"No prompt"),1),t("span",Ze,u(e.expandedPrompts.has(o.id)?"▼":"▶"),1)],10,Re),e.expandedPrompts.has(o.id)?(l(),s("div",Ge,u(o.prompt||"No prompt"),1)):m("v-if",!0)]),m(" Show agent summary when idle "),o.agentStatus==="idle"&&o.agentSummary?(l(),s("div",Xe,u(o.agentSummary),1)):m("v-if",!0),t("div",{class:"pane-interactive",onClick:n[0]||(n[0]=O(()=>{},["prevent"]))},[m(" Options Dialog (when waiting with options) "),o.agentStatus==="waiting"&&o.options&&o.options.length>0?(l(),s("div",Ye,[t("div",$e,u(o.optionsQuestion||"Choose an option:"),1),o.potentialHarm&&o.potentialHarm.hasRisk?(l(),s("div",et," ⚠️ "+u(o.potentialHarm.description),1)):m("v-if",!0),e.loadingOptions.has(o.id)?(l(),s("div",tt,[...n[13]||(n[13]=[t("div",{class:"loader-spinner"},null,-1),t("span",null,"Processing selection...",-1)])])):(l(),s("div",ot,[(l(!0),s(_,null,N(o.options,p=>(l(),s("button",{key:p.action,onClick:A=>e.selectOption(o,p),class:M(["option-button",{"option-button-danger":o.potentialHarm&&o.potentialHarm.hasRisk}]),disabled:e.loadingOptions.has(o.id)},u(p.action),11,at))),128))]))])):o.agentStatus==="analyzing"?(l(),s(_,{key:1},[m(" Analyzing (show loader) "),n[14]||(n[14]=t("div",{class:"analyzing-state"},[t("div",{class:"loader-spinner"}),t("span",null,"Analyzing...")],-1))],2112)):(l(),s(_,{key:2},[m(" Working/Idle (show prompt input) "),t("div",null,[t("div",nt,[X(t("textarea",{"onUpdate:modelValue":p=>e.promptInputs[o.id]=p,onInput:e.autoExpand,placeholder:o.agentStatus==="working"?"Queue a prompt...":"Send a prompt...",disabled:e.sendingPrompts.has(o.id),class:"prompt-textarea",rows:"1"},null,40,it),[[Y,e.promptInputs[o.id]]]),t("button",{onClick:p=>e.sendPrompt(o),disabled:!e.promptInputs[o.id]||e.sendingPrompts.has(o.id),class:"send-button",title:o.agentStatus==="working"?"Queue prompt":"Send prompt"},[e.sendingPrompts.has(o.id)?(l(),s("span",lt)):(l(),s("svg",ct,[...n[15]||(n[15]=[t("path",{d:"M425.13,28.37L30.09,423.41C11.19,441.37.34,466.2,0,492.27c-.34,26.07,9.86,51.17,28.29,69.61,18.43,18.45,43.52,28.67,69.59,28.35,26.07-.31,50.91-11.14,68.88-30.02l233.16-233.52v776.64c0,34.56,18.43,66.48,48.36,83.76,29.93,17.28,66.8,17.28,96.72,0,29.93-17.28,48.36-49.21,48.36-83.76V328.85l231.72,231.36c24.63,23.41,59.74,32.18,92.48,23.09,32.74-9.08,58.32-34.68,67.38-67.43,9.05-32.75.25-67.85-23.18-92.46L566.73,28.37C548.63,10.16,524-.04,498.33.05c-.8-.06-1.6-.06-2.4,0-.8-.06-1.6-.06-2.4,0-25.65,0-50.25,10.19-68.4,28.32h0Z"},null,-1)])]))],8,st)]),e.queuedMessages[o.id]?(l(),s("div",rt," ✓ "+u(e.queuedMessages[o.id]),1)):m("v-if",!0)])],2112))]),o.devStatus&&o.devStatus!=="stopped"?(l(),s("div",dt,[n[16]||(n[16]=t("span",{class:"status-label"},"Dev Server:",-1)),t("span",{class:M(["status-badge",o.devStatus])},u(o.devStatus),3),o.devUrl?(l(),s("a",{key:0,href:o.devUrl,target:"_blank",class:"dev-link"},"↗",8,ut)):m("v-if",!0)])):m("v-if",!0)]))),128))]))]),m(" Create Pane Dialog "),e.showCreateDialog?(l(),s("div",{key:0,class:"action-dialog-overlay",onClick:O(e.closeCreateDialog,["self"])},[t("div",gt,[n[20]||(n[20]=t("h3",null,"Create New Pane",-1)),e.createStep==="prompt"?(l(),s("div",mt,[n[17]||(n[17]=t("label",{for:"pane-prompt"},"Provide an initial prompt for your agent",-1)),X(t("textarea",{id:"pane-prompt","onUpdate:modelValue":n[1]||(n[1]=o=>e.newPanePrompt=o),placeholder:"E.g., Fix the authentication bug, Add dark mode, etc.",rows:"4",onKeydown:[$(O(e.createPane,["meta"]),["enter"]),$(O(e.createPane,["ctrl"]),["enter"])]},null,40,pt),[[Y,e.newPanePrompt]]),n[18]||(n[18]=t("div",{class:"dialog-hint"},[z(" 💡 Press "),t("kbd",null,"⌘ Enter"),z(" or "),t("kbd",null,"Ctrl Enter"),z(" to create ")],-1)),t("div",vt,[t("button",{onClick:e.closeCreateDialog,class:"dialog-btn"},"Cancel"),t("button",{onClick:e.createPane,disabled:!e.newPanePrompt.trim()||e.creatingPane,class:"dialog-btn dialog-btn-primary"},u(e.creatingPane?"Creating...":"Create Pane"),9,ht)])])):e.createStep==="agent"?(l(),s("div",ft,[n[19]||(n[19]=t("p",null,"Multiple agents available. Choose one:",-1)),t("div",yt,[(l(!0),s(_,null,N(e.availableAgents,o=>(l(),s("button",{key:o,onClick:p=>e.selectAgent(o),class:"agent-choice-button",disabled:e.creatingPane},u(o),9,bt))),128))]),t("div",{class:"dialog-buttons"},[t("button",{onClick:e.closeCreateDialog,class:"dialog-btn"},"Cancel")])])):m("v-if",!0)])])):m("v-if",!0),m(" Action Dialogs "),e.actionDialog?(l(),s("div",{key:1,class:"action-dialog-overlay",onClick:O(e.closeActionDialog,["self"])},[m(" Confirm Dialog "),e.actionDialog.type==="confirm"?(l(),s("div",_t,[t("h3",null,u(e.actionDialog.title),1),t("p",null,u(e.actionDialog.message),1),e.actionDialogLoading?(l(),s("div",kt,[...n[21]||(n[21]=[t("div",{class:"loader-spinner"},null,-1),t("span",null,"Processing...",-1)])])):(l(),s("div",wt,[t("button",{onClick:n[2]||(n[2]=o=>e.confirmAction(!1)),class:"dialog-btn",disabled:e.actionDialogLoading},"Cancel",8,Ct),t("button",{onClick:n[3]||(n[3]=o=>e.confirmAction(!0)),class:"dialog-btn dialog-btn-primary",disabled:e.actionDialogLoading},"Confirm",8,St)]))])):e.actionDialog.type==="choice"?(l(),s(_,{key:1},[m(" Choice Dialog "),t("div",Dt,[t("h3",null,u(e.actionDialog.title),1),e.actionDialog.message?(l(),s("p",Pt,u(e.actionDialog.message),1)):m("v-if",!0),e.actionDialogLoading?(l(),s("div",xt,[...n[22]||(n[22]=[t("div",{class:"loader-spinner"},null,-1),t("span",null,"Processing...",-1)])])):(l(),s("div",At,[t("div",Tt,[(l(!0),s(_,null,N(e.actionDialog.options,o=>(l(),s("button",{key:o.id,onClick:p=>e.selectChoice(o.id),class:M(["choice-option-btn",{danger:o.danger}]),disabled:e.actionDialogLoading},[t("div",Nt,u(o.label),1),o.description?(l(),s("div",Ot,u(o.description),1)):m("v-if",!0)],10,Et))),128))]),t("div",Mt,[t("button",{onClick:e.closeActionDialog,class:"dialog-btn",disabled:e.actionDialogLoading},"Cancel",8,jt)])]))])],2112)):e.actionDialog.type==="input"?(l(),s(_,{key:2},[m(" Input Dialog "),t("div",Ft,[t("h3",null,u(e.actionDialog.title),1),e.actionDialog.message?(l(),s("p",It,u(e.actionDialog.message),1)):m("v-if",!0),e.actionDialogLoading?(l(),s("div",Vt,[...n[23]||(n[23]=[t("div",{class:"loader-spinner"},null,-1),t("span",null,"Processing...",-1)])])):(l(),s("div",Lt,[X(t("input",{type:"text","onUpdate:modelValue":n[4]||(n[4]=o=>e.actionDialog.inputValue=o),placeholder:e.actionDialog.placeholder,class:"dialog-input",onKeydown:$(e.submitInput,["enter"])},null,40,zt),[[Y,e.actionDialog.inputValue]]),t("div",qt,[t("button",{onClick:e.closeActionDialog,class:"dialog-btn",disabled:e.actionDialogLoading},"Cancel",8,Ut),t("button",{onClick:e.submitInput,class:"dialog-btn dialog-btn-primary",disabled:e.actionDialogLoading},"Submit",8,Bt)])]))])],2112)):m("v-if",!0)])):m("v-if",!0)])],64)}const Ht=ke(Se,[["render",Jt],["__file","/Users/justinschroeder/Projects/dmux/frontend/src/components/Dashboard.vue"]]),Kt=Ce(Ht);Kt.mount("#app");
`,
    mimeType: 'application/javascript',
    size: 20524
  },
