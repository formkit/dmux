...
export function render() {
  return `
    ...
    <h2>Set Up an AI Provider (Recommended)</h2>
    <p>Before your first run, we recommend setting up an API key for an OpenAI-compatible provider (e.g. <a href="https://openrouter.ai" target="_blank" rel="noopener">OpenRouter</a>, OpenAI). dmux uses it to generate smart branch names from your prompts and AI-powered commit messages when merging. Without it, branch names fall back to <code>dmux-{timestamp}</code> and commit messages will be generic.</p>
    <pre><code data-lang="bash">export OPENAI_API_KEY="sk-..."</code></pre>
    <p>Add this to your shell profile (<code>~/.zshrc</code> or <code>~/.bashrc</code>) so it persists across sessions. See <a href="#configuration">Configuration</a> for provider options and details.</p>
